...
_cache/.getbins.$(kube_version_full).timestamp: clean
	...
	mv _cache/$(kube_version_full)/kubernetes/platforms/linux/amd64/e2e_node.test ./
	mv _cache/$(kube_version_full)/kubernetes/platforms/linux/amd64/kubectl ./
	rm -rf _cache/$(kube_version_full)
	touch $@

container: e2e_node.test e2e.test kubectl
...
clean:
	rm -rf _cache e2e_node.test e2e.test kubectl cluster
